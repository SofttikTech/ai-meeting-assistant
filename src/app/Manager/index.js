...
function ListMeetings() {
  ...
  const fetchAdvisors = () => {
    setLoading(true);
    fetch('http://localhost:5000/getAllAdvisors')
      .then((res) => res.json())
      .then((data) => {
        // Convert each row (array) into an object with meaningful keys
  ...
  const handleAddAdvisor = () => {
    fetch('http://localhost:5000/addAdvisor', {
      method: 'POST',
  ...
  const handleEditAdvisor = () => {
    fetch(`http://localhost:5000/editAdvisorByID/${currentID}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteAdvisor = () => {
    fetch('http://localhost:5000/deleteAdvisor', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
